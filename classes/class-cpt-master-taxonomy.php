...
class CPT_Master_Taxonomy {
	 ...
	 */
	public function __construct ( $post_type = 'cpt-master', $token = 'cpt-master-category', $singular = '', $plural = '', $args = array() ) {
		$this->post_type = $post_type;
		$this->token = esc_attr( $token );
		$this->singular = esc_html( $singular );
		$this->plural = esc_html( $plural );

		if ( '' == $this->singular ) $this->singular = __( 'Category', 'cpt-master' );
		if ( '' == $this->plural ) $this->plural = __( 'Categories', 'cpt-master' );
	...
	private function _get_default_labels () {
		return array(
			    'name'                => sprintf( _x( '%s', 'taxonomy general name', 'cpt-master' ), $this->plural ),
			    'singular_name'       => sprintf( _x( '%s', 'taxonomy singular name', 'cpt-master' ), $this->singular ),
			    'search_items'        => sprintf( __( 'Search %s', 'cpt-master' ), $this->plural ),
			    'all_items'           => sprintf( __( 'All %s', 'cpt-master' ), $this->plural ),
			    'parent_item'         => sprintf( __( 'Parent %s', 'cpt-master' ), $this->singular ),
			    'parent_item_colon'   => sprintf( __( 'Parent %s:', 'cpt-master' ), $this->singular ),
			    'edit_item'           => sprintf( __( 'Edit %s', 'cpt-master' ), $this->singular ),
			    'update_item'         => sprintf( __( 'Update %s', 'cpt-master' ), $this->singular ),
			    'add_new_item'        => sprintf( __( 'Add New %s', 'cpt-master' ), $this->singular ),
			    'new_item_name'       => sprintf( __( 'New %s Name', 'cpt-master' ), $this->singular ),
			    'menu_name'           => sprintf( __( '%s', 'cpt-master' ), $this->plural )
			  );
