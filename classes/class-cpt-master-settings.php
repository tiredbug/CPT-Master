...
/**
 * CPT_Master_Settings Class
 *
 * @class CPT_Master_Settings
 * @version	1.0.0
 * @since 1.0.0
 * @package	CPT_Master
 * @author Jeffikus
 */
final class CPT_Master_Settings {
	/**
	...
	/**
	 * Main CPT_Master_Settings Instance
	 *
	 * Ensures only one instance of CPT_Master_Settings is loaded or can be loaded.
	 *
	 * @since 1.0.0
	 * @static
	 * @return Main CPT_Master_Settings instance
	 */
		...
		if ( is_array( $input ) && 0 < count( $input ) ) {
			...
			foreach ( $input as $k => $v ) {
				...
				if ( ! method_exists( $this, $method ) ) {
					if ( true == (bool)apply_filters( 'cpt-master-validate-field-' . $fields[$k]['type'] . '_use_default', true ) ) {
				...
				if ( '' != $method ) {
					add_filter( 'cpt-master-validate-field-' . $fields[$k]['type'], array( $this, $method ) );
				}

				$method_output = apply_filters( 'cpt-master-validate-field-' . $fields[$k]['type'], $v, $fields[$k] );
	...
	public function render_field ( $args ) {
		...
		// Construct the key.
		$key 				= CPT_Master()->token . '-' . $args['section'] . '[' . $args['id'] . ']';
	...
	public function get_settings_sections () {
		...
		$settings_sections['standard-fields'] = __( 'Standard Fields', 'cpt-master' );
		$settings_sections['special-fields'] = __( 'Special Fields', 'cpt-master' );
		...
		return (array)apply_filters( 'cpt-master-settings-sections', $settings_sections );
	...
	public function get_settings_fields ( $section ) {
		...
		switch ( $section ) {
			case 'standard-fields':
				...
				$settings_fields['text'] = array(
												'name' => __( 'Example Text Input', 'cpt-master' ),
												'type' => 'text',
												'default' => '',
												'section' => 'standard-fields',
												'description' => __( 'Place the field description text here.', 'cpt-master' )
											);
				$settings_fields['textarea'] = array(
												'name' => __( 'Example Textarea', 'cpt-master' ),
												'type' => 'textarea',
												'default' => '',
												'section' => 'standard-fields',
												'description' => __( 'Place the field description text here.', 'cpt-master' )
				...
				$settings_fields['checkbox'] = array(
												'name' => __( 'Example Checkbox', 'cpt-master' ),
												'type' => 'checkbox',
												'default' => '',
												'section' => 'standard-fields',
												'description' => __( 'Place the field description text here.', 'cpt-master' )
				...
				$settings_fields['radio'] = array(
												'name' => __( 'Example Radio Buttons', 'cpt-master' ),
												'type' => 'radio',
												'default' => '',
												'section' => 'standard-fields',
												'options' => array(
																	'one' => __( 'One', 'cpt-master' ),
																	'two' => __( 'Two', 'cpt-master' ),
																	'three' => __( 'Three', 'cpt-master' )
															),
												'description' => __( 'Place the field description text here.', 'cpt-master' )
											);
				$settings_fields['select'] = array(
													'name' => __( 'Example Select', 'cpt-master' ),
													'type' => 'select',
													'default' => '',
													'section' => 'standard-fields',
													'options' => array(
			...
			case 'special-fields':
				...
				$settings_fields['select_taxonomy'] = array(
													'name' => __( 'Example Taxonomy Selector', 'cpt-master' ),
		...
		return (array)apply_filters( 'cpt-master-settings-fields', $settings_fields );
	...
	public function get_supported_fields () {
		return (array)apply_filters( 'cpt-master-supported-fields', array( 'text', 'checkbox', 'radio', 'textarea', 'select', 'select_taxonomy' ) );
	...
	public function get_value ( $key, $default, $section ) {
		$values = get_option( 'cpt-master-' . $section, array() );
	...
	public function get_settings ( $section = '' ) {
		...
		if ( 0 < count( $sections ) ) {
			foreach ( $sections as $k => $v ) {
				...
				$values = get_option( 'cpt-master-' . $v, array() );
